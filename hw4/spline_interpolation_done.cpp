...
void Spline::drawSpline(){
	...
	for(int j=0; j<splineCurves[0].curvePoints.size(); j++){
		...
		p2 = splineCurves[0].curvePoints[(j+1)%splineCurves[0].curvePoints.size()];
		v0 = p2 - p0;
		v1 = p1 - p0;
		vN = v0 * v1;
		vN = vN.normalize();
	...
	for(int j=0; j<splineCurves[splineCurves.size()-1].curvePoints.size(); j++){
		...
		p2 = splineCurves[splineCurves.size()-1].curvePoints[(j+1)%splineCurves[splineCurves.size()-1].curvePoints.size()];
		v0 = p2 - p0;
		v1 = p1 - p0;
		vN = v1 * v0;
		vN = vN.normalize();
