...
void Spline::drawSpline(){
	...
	jhm::vector orig(0, 8, -2);
	double size = 3.0;
	jhm::vector color(0.93, 0.38, 0.5);
	double alpha = 0.5;
	...
	for(int i=0; i<6; i++){
		Surface surface(norm[i],
				orig + norm[i]*size/2 - vecx[i]*size/2 - vecy[i]*size/2, 
				orig + norm[i]*size/2 + vecx[i]*size/2 - vecy[i]*size/2, 
				orig + norm[i]*size/2 - vecx[i]*size/2 + vecy[i]*size/2, 
				orig + norm[i]*size/2 + vecx[i]*size/2 + vecy[i]*size/2, 
				color, alpha);
		Segment segment0(norm[i], orig + norm[i]*size/2 - vecx[i]*size/2 - vecy[i]*size/2, orig + norm[i]*size/2 + vecx[i]*size/2 - vecy[i]*size/2);
		surface.segments.push_back(segment0);
		Segment segment1(norm[i], orig + norm[i]*size/2 - vecx[i]*size/2 + vecy[i]*size/2, orig + norm[i]*size/2 + vecx[i]*size/2 + vecy[i]*size/2);
	...
	for(int i=0; i<(int)surfaces.size(); i++){
		...
		jhm::vector clr = surfaces[i].color;
		glColor4f(clr.x(), clr.y(), clr.z(), surfaces[i].alpha);
		for(int j=0; j<(int)surfaces[i].segments.size(); j++){
			jhm::vector vN, p0, p1;
			vN = surfaces[i].segments[j].norm;
			p0 = surfaces[i].segments[j].p0;
			p1 = surfaces[i].segments[j].p1;
